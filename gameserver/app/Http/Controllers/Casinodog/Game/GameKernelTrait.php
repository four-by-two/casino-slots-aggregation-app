...
trait GameKernelTrait
{
    ...
    public function get_internal_session(string $id, string $select_data = NULL) {
        ...
        return $kernel->get_internal_session($id, $select_data);
    }

    public function get_session(string $id, string $select_data = NULL) {
        $kernel = new GameKernel();
        return $kernel->get_internal_session($id, $select_data);
    }
    ...
    public function get_game(string $gid)
    {
        $collection = collect(Gameslist::short_list());
        return $collection->where('gid', $gid)->where('active', 1)->first();
    }
